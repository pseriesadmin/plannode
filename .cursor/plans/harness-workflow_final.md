...
| **경량 제어** | 아래 **「경량화 제어」** — **불필요 로직·모듈 증가**를 GATE·스코프·검수로 견제 |

---

## 경량화 제어 — 오버 엔지니어링 견제·모듈 증가 방지

> **목표:** Plannode는 1인·내부 툴이다. **최대한 경량**을 **기본**으로 하고, “깔끔하게 보이는” **추가 추상·파일**을 **PRD·TASK·GATE 없이** 쌓지 않는다.
...
> 상세 원문: `AGENTS.md` **「경량화·오버엔지니어링 견제 제어 구조」** 표, `.cursor/harness/README.md` **최소 구현·경량화** 절.
...
| **GATE = 막힘(차단) 질문** — XP *gates*·YAGNI 정신 [en-yao/xp-gates 요지](https://github.com/en-yao/xp-gates) | `GATE A` “제외/PRD와 모순 없나”, `GATE C` “범위·경량(신규 모듈) 괜찮나” = **순차**로 실패 시 다음 단계 **진행 안 함**과 같음(사람이 판). |
...
- **“층 줄이기” (Kruczek)** → `GP-12`·GATE C **(경량) 문항**·`promptor` YAGNI 한 줄 = **툴을 늘리지 않고** **질문으로** 측정.
- **YAGNI·Simple design** (XP) → P-6.5 + **경량화** 규칙 1~3번과 **동어**.
