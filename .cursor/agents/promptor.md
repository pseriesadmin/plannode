...
# promptor — Plannode 아젠다 분석 에이전트 v1.2
...
6. 아젠다가 **뷰·출력·IA 그리드·xlsx·캔버스 재연결**을 포함하면 — `.cursor/plans/plannode-ai-logic-v4.md` **§4.0**·**§3** (`OutputIntent` 표)·**§4.0.1** (캔버스·「노드」 라벨)
7. 아젠다가 **UI·스타일·모달·툴바·노드 카드·반응형·브랜드 색** 등 화면을 건드리면 — `.cursor/rules/plannode-ui-identity.mdc` (**표준 디자인 시스템**: Primary `#631EED`, 타이포·레이어·파일럿 DOM 계약 요지)
```
...
**트리뷰 보호 (필수 한 줄):** 아젠다가 트리·파일럿·`#V-TREE`·브리지·노드 SSoT를 건드리면 `plan-output` **포함/제외**에 **회귀 검증**·저해 방지 조건을 1줄 넣는다. 부가 뷰만 다룰 때도 **캔버스·`.view` 전환 저해 금지**를 **제외** 또는 **주의**에 명시한다 (`AGENTS.md` **GP-13**).

---

## P-3.5. v4 보기·출력 정본 동기 (해당 시)

> [plannode-ai-logic-v4.md §4.0](../plans/plannode-ai-logic-v4.md) · **§3** `OutputIntent` 3종 표와 **내부 식별자** vs **사용자 메뉴명** 매핑을 `plan-output.md`에 **1문장 이상** 반영한다. **§5.0.1** M2 ID 후보(`M2-EXPORT-XLSX` 등)가 걸리면 Step3 `TASK.md`의 각 `NOW` 제목에 **동일 문자열**을 텍스트로 포함한다(v4 §1.2.4). **사용자 라벨「노드」**은 §4.0·§4.0.1과 혼동 없이 쓴다.

**해당 없음:** 아젠다가 보기/출력/내보내기/그리드와 무관하면 `"P-3.5 해당 없음"` 한 줄.
...
5. **편집·저장 귀속 (기능명세·IA 그리드):** 저장이 **트리 SSoT**(`treeText`/`nodes`) vs **메타데이터** vs **PRD §11 스냅샷** 중 어디에 귀속하는지 **택일 전제**를 `plan-output` **P-4.5**에 1문장 명시한다. 미결은 **GATE B 조건**으로 남긴다.
...
**디자인 시스템 (UI 아젠다 시 필수 한 줄):** Step3 `TASK.md`·이후 **@harness-executor** 또는 **서브에이전트**에 넘길 지시문에는 **`.cursor/rules/plannode-ui-identity.mdc`** 를 **유일한 UI 토큰·레이아웃·DOM 계약 참조**로 명시한다(색·폰트·z-index·`:global` 계약). `plan-output.md` **P-6** 또는 **참고 파일**에 동일 경로를 적어 하위 에이전트가 빠지지 않게 한다.
...
[ ] v4 §4.0·§4.0.1·§3 (`OutputIntent`) 요지가 아젠다와 연결됐는가(뷰·출력 해당 시)?
[ ] UI·스타일 변경 시 `plannode-ui-identity.mdc` 가 plan-output·P-6에 인용됐는가(해당 시)?
[ ] 핵심 위험 요소 대응에 동의하는가?
...
*promptor.md v1.2 | Plannode | Harness Flow v1.0 | 2026-04-23 (KST)*
