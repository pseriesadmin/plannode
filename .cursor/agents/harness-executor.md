...
```
실행 스코프 (모든 NOW 공통 — 위반 시 구현 중단·되돌림):
...
Step 0-2. AGENTS.md 황금 원칙 GP-1~GP-12 재로드 (GP-12: 오버·부채)
...
```
0. 실행 스코프: 요청한 파일·섹션만 수정. 다른 컴포넌트·전역 스타일·파일럿 DOM id 변경 금지. (TASK.md NOW·plan-output 포함 범위 준수)
...
```
구현 중 준수사항:
- 실행 스코프: 요청한 파일·섹션만 수정. 다른 컴포넌트·전역 스타일·파일럿 DOM id 변경 금지.
- 파일 1개 수정 후 즉시 결과 확인 (대규모 변경 금지)
- 기존 코드 삭제 전 Stephen에게 확인 요청
- AGENTS.md 금지 패턴 목록과 대조 후 코드 생성
- AGENTS.md GP-12: 오버 엔지니어링(범위·PRD外 선제 일반화, 불필요한 추상) 지양
- 기술부채: console.log(불필요)·TODO·any·미사용 import·무분별 신규 의존성 남기지 않음 — @qa 2단계 기준
- console.log 잔류 금지
- any 타입 사용 금지
```
...
[ ] (경량) 이번 NOW에 **없는** 신규 모듈/파일·**불필요한** 추상·래퍼를 **추가하지 않았는가**? (필요 시 `AGENTS` 경량화 제어 3단계)
...
[ ] 코드가 아젠다 의도와 일치하는가?
[ ] 범위 밖 기능이 추가되지 않았는가?
[ ] (PRD: 있을 때) M#·F#-#·IA/LLM 구분이 지켜졌는가?
[ ] (경량) plan-output·TASK **없는** 신규 모듈/불필요 추상을 **늘리지 않았는가**?
