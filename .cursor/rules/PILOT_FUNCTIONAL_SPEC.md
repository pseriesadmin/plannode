...
- **`nodes` 트리 데이터와 무관**(SSoT 아님). 브라우저 `localStorage`로 구분한다.
  - **`plannode.nodeMapLayout.v1`** — JSON 객체: **프로젝트 id →** `right` | `topdown`. 열린 프로젝트마다 **해당 id**에 저장·복원(다른 프로젝트로 바꿔도 이전 프로젝트의 배치가 섞이지 않음). 루트 노드 우클릭 메뉴·생성 직후 적용 등으로 갱신. 구버전 **`plannode.nodeMapLayout`**(전역 한 값)은 최초 1회 `plannode_projects_v3`의 프로젝트 id에 맵핑한 뒤 제거한다.
...
- 항목: 편집(이름·설명·**배지**는 모달에서), 하위 추가, 위치 초기화(`mx/my=null`), (부모 있을 때만) 삭제. **2026-06:** 우클릭 3트랙 배지 `bgt` 토글 **제거** — 협업 동기화 정본은 `showEdit` + `BADGE_STRUCTURE_OPS`(아키텍처 §10.10.1).
- 루트 노드: 노드맵 배치(우측·하위분포) 메뉴 추가.
...
| 컨텍스트 배지 | ~~파일럿 메뉴 내 토글~~ **2026-06 제거** — 배지는 모달만 | SvelteKit 동일(편집/추가/초기화/삭제) | **정합** · collab §10.10.1 |
